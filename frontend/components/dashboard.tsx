...
export function Dashboard({ data }: DashboardProps) {
  ...
  const summaryCards = [
    {
      label: "Total Emiten",
      value: String(summary.totalEmiten),
    ...
    },
    {
      label: "Bobot Dominan",
      value: dominanBobot,
      sub: `Entropy ${(dominanWeight * 100).toFixed(0)}%`,
          ...
          <Card className="border-slate-200 shadow-sm">
            ...
            <CardContent className="px-6 pb-6">
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                {[
                  { key: "EPS", label: "Earnings Per Share", type: "Benefit", weight: entropyWeights.eps, bar: "bg-violet-500" },
                  { key: "DER", label: "Debt to Equity Ratio", type: "Cost", weight: entropyWeights.der, bar: "bg-amber-400" },
                  { key: "ROA", label: "Return on Assets", type: "Benefit", weight: entropyWeights.roa, bar: "bg-blue-500" },
                  { key: "PBV", label: "Price to Book Value", type: "Cost", weight: entropyWeights.pbv, bar: "bg-slate-300" },
                ].map((c) => (
